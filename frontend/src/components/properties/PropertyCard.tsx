...
const PropertyCard: React.FC<PropertyCardProps> = ({
  ...
  return (
    <Link to={`/property/${id}`} className="group block outline-none focus-visible:ring-2 focus-visible:ring-[#2563EB] rounded-2xl">
      <article className="bg-white rounded-2xl overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.06),0_1px_2px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.10)] transition-shadow duration-300">
        ...
        <div className="relative aspect-[4/3] overflow-hidden bg-[#E2E8F0]">
          ...
          >
            <Heart
              className={`w-4 h-4 transition-colors duration-200 ${favorited ? 'text-[#2563EB] fill-[#2563EB]' : 'text-white'}`}
            />
          </button>
        ...
        <div className="px-4 py-4">
          {/* Name */}
          <h3 className="font-fraunces text-base font-semibold text-[#0F172A] leading-snug mb-1.5 text-wrap-balance line-clamp-2">
            {name}
          </h3>

          {/* Location */}
          <div className="flex items-center gap-1 mb-3">
            <MapPin className="w-3.5 h-3.5 text-[#2563EB] shrink-0" aria-hidden />
