          ...
          {images.map((img, i) => (
            <button
              ...
              className={`w-12 h-8 rounded-md overflow-hidden transition-all ${
                i === current ? 'ring-2 ring-[#2563EB] opacity-100' : 'opacity-40 hover:opacity-70'
              }`}
            >
  ...
  return (
    <>
      <div className="relative h-[60vh] min-h-[380px] bg-[#1C1B1A] overflow-hidden">
        ...
        <button
          onClick={() => open(0)}
          className="absolute bottom-4 right-4 flex items-center gap-2 bg-white/90 backdrop-blur-sm hover:bg-white text-[#0F172A] font-manrope font-semibold text-sm px-4 py-2 rounded-xl shadow-md transition-all active:scale-[0.96]"
        >
