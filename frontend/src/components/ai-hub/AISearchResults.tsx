...
const SOURCE_META: Record<string, { label: string; cls: string }> = {
  '99acres':    { label: '99acres',     cls: 'bg-blue-50  text-blue-600  border-blue-200'  },
  ...
  return (
    <div
      className={`
        ...
        ${isComparing
          ? 'shadow-[0_0_0_2px_#2563EB,0_8px_24px_-4px_rgba(37, 99, 235,0.2)]'
    ...
    >
      ...
      <div className={`absolute top-0 inset-x-0 h-0.5 ${isBest ? 'bg-gradient-to-r from-[#2563EB] via-amber-400 to-[#2563EB]' : 'bg-gradient-to-r from-[#2563EB]/40 to-amber-400/40'}`} />
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        {/* Building name */}
        <h3 className="font-syne text-[18px] font-bold text-[#0F172A] mb-0.5 leading-tight line-clamp-2 [text-wrap:balance]">
        ...
        <div className="flex items-center gap-1.5 mb-4">
          <MapPin className="w-3 h-3 text-[#2563EB]/60 shrink-0" />
        ...
        <div className="flex items-center justify-between bg-[#FFFFFF] rounded-xl px-4 py-3 mb-3 shadow-[0_0_0_1px_rgba(0,0,0,0.05)]">
          <div>
            ...
            <p className="font-manrope font-extrabold text-[#2563EB] text-[20px] leading-none tabular-nums">
        ...
        {insight?.one_line_insight && (
          <div className="relative bg-gradient-to-br from-[#FDF6F3] to-[#FAF4F0] border border-[#2563EB]/20 rounded-xl px-4 py-3 mb-3 overflow-hidden">
            <div className="absolute top-2 right-2">
              <Sparkles className="w-3.5 h-3.5 text-[#2563EB]/30" />
            </div>
            <p className="font-space-mono text-[9px] text-[#2563EB]/60 font-bold uppercase tracking-widest mb-1.5">
              AI Insight
            </p>
            ...
            {hasMatchScore && (
              <div className="flex items-center gap-2 mt-2.5">
                <div className="flex-1 h-1 bg-[#E2E8F0] rounded-full overflow-hidden">
                  <motion.div
                    className="h-full bg-gradient-to-r from-[#2563EB] to-amber-400 rounded-full"
                    initial={{ width: 0 }}
        ...
        <div className="flex gap-2 mt-4 pt-4 border-t border-[#E2E8F0]/50">
          {property.property_url ? (
            <a
              ...
              className="flex-1 inline-flex items-center justify-center gap-1.5 bg-[#2563EB] hover:bg-[#1E40AF] text-white font-manrope font-semibold text-sm py-2.5 rounded-xl transition-[background-color,box-shadow] duration-150 shadow-sm shadow-[#2563EB]/20 active:scale-[0.96]"
            >
          ...
          <button
            ...
            className={`px-4 py-2.5 font-manrope font-semibold text-sm rounded-xl transition-[background-color,border-color,color] duration-150 border active:scale-[0.96] ${
              isComparing
                ...
                : canCompare
                  ? 'border-[#E2E8F0] text-[#6B7280] hover:border-[#2563EB]/50 hover:text-[#2563EB]'
...
const ComparisonModal: React.FC<{
  ...
  const getCellValue = (property: ScrapedProperty, key: string): React.ReactNode => {
    ...
    switch (key) {
      case 'price':
        return <span className="font-bold text-[#2563EB] tabular-nums">{property.price || '—'}</span>;
      case 'price_per_sqft':
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E2E8F0] shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-syne text-xl font-bold text-[#0F172A]">Compare Properties</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[#FFFFFF] text-[#9CA3AF] hover:text-[#0F172A] transition-[background-color,color] active:scale-[0.96]"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
              ...
              <tr className="border-b-2 border-[#E2E8F0]">
                ...
                {items.map((p, i) => (
                  <th key={i} className="px-5 py-4 text-left min-w-[220px]">
                    <p className="font-syne font-bold text-[#0F172A] text-[15px] leading-tight">{p.building_name || 'Property'}</p>
    ...
    <div className="py-16">
      ...
      <div className="flex justify-center mb-10">
        <div className="relative w-20 h-20">
          {/* Outer pulse rings */}
          <div className="absolute inset-0 rounded-full bg-[#2563EB]/10 animate-ping" style={{ animationDuration: '1.8s' }} />
          <div className="absolute inset-2 rounded-full bg-[#2563EB]/15 animate-ping" style={{ animationDuration: '2.4s', animationDelay: '0.4s' }} />
          {/* Core orb */}
          <div className="absolute inset-4 rounded-full bg-gradient-to-br from-[#2563EB] to-amber-400 shadow-[0_0_24px_rgba(37, 99, 235,0.5)]" />
          {/* Inner shimmer */}
      ...
      <div className="text-center mb-6">
        <AnimatePresence mode="wait">
          <motion.p
            ...
            className="font-space-mono text-[10px] text-[#2563EB] font-bold uppercase tracking-widest mb-2"
          >
            {stage === 'analyzing' ? 'AI Analysis' : 'Live Search'}
          </motion.p>
        </AnimatePresence>
        <h2 className="font-syne text-2xl font-bold text-[#0F172A] mb-1 [text-wrap:balance]">
      ...
      <div className="max-w-sm mx-auto bg-white rounded-2xl shadow-[0_0_0_1px_rgba(0,0,0,0.06),0_4px_12px_-2px_rgba(0,0,0,0.08)] overflow-hidden">
        ...
        <div className="px-4 py-4 space-y-2.5">
          ...
          <div className="flex items-start gap-2.5">
            <span className="font-space-mono text-[10px] text-[#2563EB] mt-0.5 shrink-0">›</span>
            <AnimatePresence mode="wait">
              <motion.span
                key={displayMsg}
                ...
                className="font-space-mono text-[11px] text-[#0F172A]"
              >
                {displayMsg}
              </motion.span>
            </AnimatePresence>
          ...
          <div className="flex items-center gap-2.5 pl-5">
            <span className="font-space-mono text-[11px] text-[#2563EB] animate-pulse">▋</span>
          </div>
        </div>
      </div>
        ...
        {[1, 2, 3, 4].map(i => (
          <div key={i} className="bg-white rounded-2xl p-5 shadow-[0_0_0_1px_rgba(0,0,0,0.06)] animate-pulse">
            ...
            <div className="flex gap-2 pt-3 border-t border-[#E2E8F0]/30">
              <div className="flex-1 h-9 bg-[#2563EB]/15 rounded-xl" />
  ...
  if (error) {
    return (
      <div className="py-10 text-center">
        ...
        <h3 className="font-syne text-2xl text-[#0F172A] mb-2">Search Failed</h3>
  ...
  if (properties.length === 0) {
    return (
      <div className="py-10 text-center">
        ...
        <h3 className="font-syne text-2xl text-[#0F172A] mb-2">No Properties Found</h3>
    ...
    <>
      <div>
        ...
        <div className="mb-7 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <div className="font-space-mono text-[10px] text-[#2563EB] font-bold uppercase tracking-widest mb-2 flex items-center gap-2">
              <TrendingUp className="w-3 h-3" />
              Live AI Results
            </div>
            <h2 className="font-syne text-3xl font-bold text-[#0F172A] mb-1.5 [text-wrap:balance]">
              Properties in {city}
            </h2>
            <p className="font-manrope text-sm text-[#6B7280]">
              <span className="tabular-nums font-semibold text-[#0F172A]">{properties.length}</span>{' '}
              {properties.length === 1 ? 'match' : 'matches'} found
              {aiMatchCount > 0 && (
                <> · <span className="tabular-nums font-semibold text-[#2563EB]">{aiMatchCount}</span> with AI insights</>
              )}
            </p>
          </div>
        </div>
              ...
              <div className="max-w-[1200px] mx-auto px-6 py-4 flex items-center gap-4">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  {compareList.map((p, i) => (
                    <div key={i} className="flex items-center gap-2 bg-[#FFFFFF] border border-[#E2E8F0] rounded-xl px-3 py-1.5 min-w-0 shadow-[0_0_0_1px_rgba(0,0,0,0.04)]">
                      <span className="font-manrope text-[13px] font-semibold text-[#0F172A] truncate max-w-[140px]">
                        {p.building_name || 'Property'}
                      </span>
                      <button
                        type="button"
                        onClick={() => toggleCompare(p)}
                        className="shrink-0 text-[#9CA3AF] hover:text-[#2563EB] transition-[color] active:scale-[0.96]"
                      >
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 bg-[#2563EB] hover:bg-[#1E40AF] text-white font-manrope font-semibold text-sm px-5 py-2.5 rounded-xl transition-[background-color] duration-150 shadow-sm shadow-[#2563EB]/25 active:scale-[0.96]"
                  >
