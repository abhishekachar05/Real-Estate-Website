...
const Navbar: React.FC = () => {
  ...
  return (
    ...
    <a
      href="#main-content"
      className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-[9999] focus:bg-[#2563EB] focus:text-white focus:font-manrope focus:font-bold focus:px-4 focus:py-2 focus:rounded-lg focus:shadow-lg"
    >
    ...
    >
      <div className="max-w-[1280px] mx-auto px-8 flex items-center justify-between h-20">
        ...
        <div className="hidden md:flex items-center gap-3">
          {isAuthenticated && user ? (
            <>
              <Link
                to="/add-property"
                className="bg-[#2563EB] text-white font-manrope font-bold px-5 py-2 rounded-xl hover:bg-[#1D4ED8] transition-[background-color,box-shadow] hover:shadow-md active:scale-[0.96] transition-transform"
              >
                + List Property
              </Link>
              ...
              <div className="relative" ref={userMenuRef}>
                ...
                >
                  <div className="w-8 h-8 rounded-lg bg-[#2563EB] text-white font-manrope font-bold text-xs flex items-center justify-center shrink-0">
                    {initials}
                  </div>
                  <span className="font-manrope font-semibold text-[#0F172A] max-w-[100px] truncate">
                    {user.name.split(' ')[0]}
                  </span>
                ...
                {isUserMenuOpen && (
                  <div className="absolute right-0 top-full mt-2 w-52 bg-white border border-[#E2E8F0] rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.08),0_1px_4px_rgba(0,0,0,0.04)] py-2 z-50">
                    <div className="px-4 py-2.5 border-b border-[#F3F0EC] mb-1">
                      ...
                      <p className="font-manrope text-sm font-semibold text-[#0F172A] truncate">{user.email}</p>
                    </div>
                    {[
                      ...
                      <Link
                        ...
                        className={`flex items-center gap-2.5 px-4 py-2.5 font-manrope text-sm transition-[background-color,color] mx-1 rounded-xl ${
                          isActive(to) ? 'text-[#2563EB] font-semibold bg-[#EFF6FF]' : 'text-[#374151] hover:bg-[#EFF6FF] hover:text-[#2563EB]'
                        }`}
                      >
            ...
            <>
              <Link
                to="/signin"
                className="font-manrope font-semibold text-[#374151] hover:text-[#2563EB] transition-[color] px-4 py-2"
              >
                Sign In
              </Link>
              <Link
                to="/signup"
                className="bg-[#2563EB] text-white font-manrope font-bold px-6 py-2 rounded-xl hover:bg-[#1D4ED8] transition-[background-color,box-shadow] hover:shadow-md active:scale-[0.96]"
              >
                Sign Up
              </Link>
            </>
        ...
        {/* Mobile menu button */}
        <button
          className="md:hidden p-2 text-[#374151] hover:text-[#2563EB] transition-[color]"
          ...
          {navLinks.map((link) => (
            <Link
              ...
              className={`font-manrope text-base py-2.5 transition-[color] ${
                isActive(link.path) ? 'text-[#2563EB] font-semibold' : 'text-[#374151]'
              }`}
          ...
          <div className="border-t border-gray-100 mt-2 pt-3 flex flex-col gap-1">
            {isAuthenticated && user ? (
              <>
                ...
                <Link to="/dashboard" className="font-manrope text-base py-2.5 text-[#374151] hover:text-[#2563EB] transition-[color]" onClick={closeMobileMenu}>Dashboard</Link>
                <Link to="/my-listings" className="font-manrope text-base py-2.5 text-[#374151] hover:text-[#2563EB] transition-[color]" onClick={closeMobileMenu}>My Listings</Link>
                <Link
                  to="/add-property"
                  className="mt-2 bg-[#2563EB] text-white font-manrope font-bold text-sm px-6 py-3 rounded-lg hover:bg-[#1D4ED8] transition-all text-center"
                  onClick={closeMobileMenu}
                >
                  + List Property
                </Link>
