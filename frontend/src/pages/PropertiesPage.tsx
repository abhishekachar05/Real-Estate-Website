  ...
  return (
    <div className="bg-[#FFFFFF] min-h-screen">
      ...
      <div className="max-w-[1440px] mx-auto px-6 pt-8 pb-2">
        <h1 className="font-fraunces text-3xl font-semibold text-[#0F172A]">All Properties</h1>
      </div>
      ...
      {error && !loading && (
        <div className="flex items-center justify-center py-24">
          <div className="text-center">
            <span className="material-icons text-4xl text-[#2563EB] mb-4 block">error_outline</span>
            <p className="font-manrope text-[#374151] mb-4">{error}</p>
            <button
              onClick={() => window.location.reload()}
              className="bg-[#2563EB] text-white font-manrope font-bold px-6 py-2.5 rounded-xl hover:bg-[#1D4ED8] active:scale-[0.96] transition-all"
            >
              Retry
            </button>
          </div>
      ...
      {!loading && !error && filteredProperties.length === 0 && (
        ...
        >
          <div className="text-center">
            ...
            <p className="font-fraunces text-xl text-[#0F172A] mb-2">No properties found</p>
