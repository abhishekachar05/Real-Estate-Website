  ...
  return (
    <form onSubmit={handleSaveProfile} className="max-w-lg space-y-8">
      ...
      <section className="bg-white border border-[#E2E8F0] rounded-2xl p-6">
        <h3 className="font-syne font-bold text-base text-[#0F172A] mb-5">Profile</h3>
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full border border-[#E2E8F0] rounded-xl px-4 py-2.5 font-manrope text-sm text-[#0F172A] focus:outline-none focus:border-[#2563EB] transition-[border-color]"
      ...
      {/* Password */}
      <section className="bg-white border border-[#E2E8F0] rounded-2xl p-6">
        <h3 className="font-syne font-bold text-base text-[#0F172A] mb-1">Change Password</h3>
      ...
      <button
        type="submit"
        disabled={saving}
        className="bg-[#2563EB] font-manrope font-bold text-sm text-white px-6 py-2.5 rounded-xl hover:bg-[#1D4ED8] transition-[background-color] disabled:opacity-60 disabled:cursor-not-allowed"
      >
  ...
  return (
    <div className="min-h-screen bg-[#FFFFFF] flex flex-col">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 pt-28 pb-16">
        {/* Header */}
        <div className="mb-6">
          <h1 className="font-syne font-bold text-3xl text-[#0F172A] mb-1">
            ...
            <button
              ...
              className={`font-manrope font-semibold text-sm px-5 py-2.5 -mb-px border-b-2 transition-[color,border-color] capitalize ${
                activeTab === tab
                  ...
                  : 'border-transparent text-[#4B5563] hover:text-[#0F172A]'
              }`}
            >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          {stats.map(({ label, value, icon }) => (
            ...
            >
              <span className="font-material-icons text-[#2563EB] text-2xl" aria-hidden="true">{icon}</span>
              <div className="font-syne font-bold text-2xl text-[#0F172A] mt-2 tabular-nums">
                {fetchLoading ? '—' : value}
              </div>
        ...
        <div className="flex flex-wrap gap-3 mb-10">
          <Link
            to="/my-listings"
            className="bg-white border border-[#E2E8F0] font-manrope font-semibold text-sm text-[#0F172A] px-5 py-2.5 rounded-xl hover:border-[#2563EB] hover:text-[#2563EB] transition-[border-color,color]"
          >
            Manage My Listings
          </Link>
          <Link
            to="/add-property"
            className="bg-[#2563EB] font-manrope font-bold text-sm text-white px-5 py-2.5 rounded-xl hover:bg-[#1D4ED8] transition-[background-color]"
          >
            + List a Property
          </Link>
          <Link
        ...
        {/* Appointments */}
        <section>
          <h2 className="font-syne font-bold text-xl text-[#0F172A] mb-4">My Appointments</h2>
          ...
          {fetchLoading ? (
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-2xl p-10 text-center">
              <span className="font-material-icons text-4xl text-[#2563EB]/40" aria-hidden="true">event_busy</span>
              <p className="font-manrope text-sm text-[#4B5563] mt-3 mb-5">
                No appointments yet. Book a viewing from any property page.
              </p>
              <Link
                to="/properties"
                className="inline-block bg-[#2563EB] font-manrope font-bold text-sm text-white px-5 py-2.5 rounded-xl hover:bg-[#1D4ED8] transition-[background-color]"
              >
                Browse Properties
              </Link>
            </div>
                  ...
                  >
                    ...
                    {apt.propertyId?.image?.[0] ? (
                      ...
                      <div className="w-full sm:w-20 h-32 sm:h-16 bg-[#FFFFFF] border border-[#E2E8F0] rounded-xl flex items-center justify-center shrink-0">
                        <span className="font-material-icons text-[#2563EB]/40" aria-hidden="true">home</span>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {apt.meetingLink && apt.status === 'confirmed' && (
                        <a
                          ...
                          className="font-manrope font-semibold text-xs text-white bg-[#0F172A] px-4 py-2 rounded-lg hover:bg-[#2563EB] transition-[background-color]"
                        >
                          Join Meeting
                        </a>
                      )}
                      {apt.propertyId && (
                        <Link
                          ...
                          className="font-manrope font-semibold text-xs text-[#0F172A] border border-[#E2E8F0] px-4 py-2 rounded-lg hover:border-[#2563EB] hover:text-[#2563EB] transition-[border-color,color]"
                        >
                          View
                        </Link>
                      )}
