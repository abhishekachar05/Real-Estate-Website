...
const getSiteUrl = () => (typeof window !== 'undefined' ? window.location.origin : '');
const DEFAULT_TITLE = 'Propert-E - AI-Powered Luxury Real Estate | Find Your Dream Home';
...
export function useSEO({ title, description, image, url, type = 'website' }: SEOProps) {
  useEffect(() => {
    const fullTitle = title ? `${title} | Propert-E` : DEFAULT_TITLE;
    ...
    return () => {
      ...
      setOG('og:title', 'Propert-E - AI-Powered Luxury Real Estate');
      setOG('og:description', DEFAULT_DESCRIPTION);
      setOG('og:image', DEFAULT_IMAGE);
      setOG('og:url', siteUrl);
      setOG('og:type', 'website');
      setTwitter('twitter:title', 'Propert-E - AI-Powered Luxury Real Estate');
