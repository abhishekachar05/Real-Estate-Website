...
The overview array must have ${preparedProperties.length} objects — one per property, ranked by match_score descending:
{
  ...
  "recommendations": [
    ...
    // The schema is large: allow enough output for all eight complete property objects.
    return this.generateText(prompt, SYSTEM_PROMPT, { maxTokens: 4500, timeoutMs: 90_000 });
  }
...
/**
 * Factory � build an AIService using NVIDIA NIM only.
 * The first and third parameters remain for compatibility with existing callers.
 */
export function createAIService(
  _githubKey = null,
  nvidiaKey = null,
  _githubModelsConfig = null,
  nvidiaModelsConfig = null,
) {
  const providers = [];

  if (nvidiaKey) {
    providers.push(new NvidiaNimProvider(nvidiaKey, nvidiaModelsConfig));
  }

  if (!providers.length) {
    throw new Error('[AIService] NVIDIA AI provider key is required.');
  }

  logger.info('AIService: provider chain', {
