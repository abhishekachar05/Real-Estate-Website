...
// Nemotron Super is the supported chat model used for property analysis.
...
export class NvidiaNimProvider extends LLMProvider {
    ...
    constructor(apiKey, modelsConfig = null) {
        ...
        this.apiKey = apiKey;
        // Existing database records may still point to the timed-out Lightning model.
        this.models = (modelsConfig || DEFAULT_MODELS).map(model =>
            model.modelId === 'nvidia/nemotron-3.5-lightning-30b-a3b'
                ? { ...model, modelId: DEFAULT_MODELS[0].modelId, slug: DEFAULT_MODELS[0].slug }
                : model
        );
            ...
            for (let attempt = 0; attempt < 2; attempt++) {
                try {
                    ...
                    if (retryable && attempt === 0) {
                        logger.warn('NvidiaNim transient failure, retrying', {
                            slug: model.slug,
