 ...
 * The NVIDIA NIM key is server-side only — loaded from the environment.
...
async function resolveServices(req) {
    ...
    try {
        const { nvidia } = await resolveModelsByProvider();
    ...
    return {
        aiService:        createAIService(null, serverNvidiaKey, null, nvidiaModelsConfig),
...
export const searchProperties = async (req, res) => {
    ...
    try {
        ...
        const cacheKey = `search:v4:${city}:${locality}:${bhk}:${minPrice}:${maxPrice}:${propertyCategory}:${propertyType}:${possession}:limit${limit}:m${modelSlug}`;
        ...
        try {
            result = await coalesce(cacheKey, async () => {
                ...
                if (!analysis.error) {
                    ...
                    logger.warn('Skipping cache for failed AI analysis', { key: cacheKey.substring(0, 50) });
                }
                return payload;
            });
        } catch (coalesceError) {
...
/** GET /api/user/properties/:id — get a single owned listing (for edit form) */
export const getUserListingById = async (req, res) => {
    try {
        ...
        if (!property.postedBy || property.postedBy.toString() !== req.user._id.toString()) {
            return res.status(403).json({ success: false, message: 'Not authorised to view this listing' });
        }

        res.json({ success: true, property });
    } catch (error) {
        logger.error("Error fetching user listing by id", { error: error.message, stack: error.stack });
